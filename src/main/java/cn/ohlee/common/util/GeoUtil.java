...
public final class GeoUtil {
    ...
    public static Double[] baiduDecrypt(double bd_lat, double bd_lon) {
        ...
        Double[] marsCoodinate = new Double[] {};
...
//		System.out.println(DistanceOfTwoPoints(116.45475, 40.000869, 127.86436,
//				41.584709, GaussSphere.Beijing54));
        ...
        int radius = 1000;
        double dis = DistanceOfTwoPoints(lat1, lon1, lat2, lon2, GaussSphere.Beijing54);
    ...
    /**
     * 判断两个点的距离是否小于半径
     * 即判断一个点是否在以另一个点为圆心，radius为半径的圆中
     * 做商圈验证用
     * @param lng1
     * @param lat1
     * @param lng2
     ...
     * @return
     */
    public static Boolean isInRadius(double lng1, double lat1,
                                     double lng2, double lat2, double radius){
        double dis = DistanceOfTwoPoints(lng1, lat1, lng2, lat2, GaussSphere.Beijing54);
        return radius >= dis;
    }
}
